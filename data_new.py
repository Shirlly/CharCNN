# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 13:47:26 2016
...
def BASF_char(num_train, word_len, sent_len):
        ...
        for i in range(len(X)):
            if type(X[i]) is str and type(y[i]) is str:
                if X[i].strip() and y[i].strip():
                    ...
                    y_label.append(y[i])
    '''
    X_text = preprocessText(X)
    data = CharNumberEncoder(X_text, word_len=word_len, sent_len=sent_len)
    ...
    return train_X, valid_X, train_ys, valid_ys, components
